<script lang="ts">
  export let kpis: any = {};
  
  function formatDelta(delta: number): string {
    if (delta === 0) return '—';
    const sign = delta > 0 ? '+' : '';
    return `${sign}${delta.toFixed(1)}`;
  }
  
  function getDeltaColor(delta: number): string {
    if (delta > 0) return 'text-success-600';
    if (delta < 0) return 'text-error-600';
    return 'text-text-secondary';
  }
</script>

<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-gallery">
  <div class="bg-gradient-to-br from-brand-50 to-brand-100 p-card rounded-card border border-brand-200 shadow-card">
    <div class="flex items-center justify-between">
      <div>
        <p class="text-body font-medium text-brand-600 uppercase tracking-wide">Avg Rating</p>
        <p class="text-display font-bold text-brand-800 mt-2">{kpis.avgRating?.toFixed(1) ?? '—'}</p>
        <p class="text-body text-brand-700 mt-1">out of 10</p>
      </div>
      <div class="w-12 h-12 bg-brand-600 rounded-full flex items-center justify-center">
        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path>
        </svg>
      </div>
    </div>
  </div>
  
  <div class="bg-gradient-to-br from-success-50 to-success-100 p-card rounded-card border border-success-200 shadow-card">
    <div class="flex items-center justify-between">
      <div>
        <p class="text-body font-medium text-success-600 uppercase tracking-wide">Total Reviews</p>
        <p class="text-display font-bold text-success-800 mt-2">{kpis.reviewCount ?? 0}</p>
        <p class="text-body {getDeltaColor(kpis.last30dDelta)} mt-1">
          {formatDelta(kpis.last30dDelta)} this month
        </p>
      </div>
      <div class="w-12 h-12 bg-success-600 rounded-full flex items-center justify-center">
        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
        </svg>
      </div>
    </div>
  </div>
  
  <div class="bg-gradient-to-br from-accent-50 to-accent-100 p-card rounded-card border border-accent-200 shadow-card">
    <div class="flex items-center justify-between">
      <div>
        <p class="text-body font-medium text-accent-600 uppercase tracking-wide">5-Star Rate</p>
        <p class="text-display font-bold text-accent-800 mt-2">{((kpis.fiveStarPct ?? 0) * 100).toFixed(0)}%</p>
        <p class="text-body text-accent-700 mt-1">excellent reviews</p>
      </div>
      <div class="w-12 h-12 bg-accent-600 rounded-full flex items-center justify-center">
        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path>
        </svg>
      </div>
    </div>
  </div>
  
  <div class="bg-gradient-to-br from-warning-50 to-warning-100 p-card rounded-card border border-warning-200 shadow-card">
    <div class="flex items-center justify-between">
      <div>
        <p class="text-body font-medium text-warning-600 uppercase tracking-wide">Growth</p>
        <p class="text-display font-bold text-warning-800 mt-2 {getDeltaColor(kpis.last30dDelta)}">
          {formatDelta(kpis.last30dDelta)}
        </p>
        <p class="text-body text-warning-700 mt-1">last 30 days</p>
      </div>
      <div class="w-12 h-12 bg-warning-600 rounded-full flex items-center justify-center">
        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z"></path>
        </svg>
      </div>
    </div>
  </div>
</div>
